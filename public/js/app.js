    ...
    const PLATFORM_LABELS = { openclaw: 'OpenClaw', codex: 'Codex', 'claude-code': 'Claude Code', hermes: 'Hermes', omp: 'OMP' };
    ...
    function probePlatformSessionCounts() {
      ...
      Promise.all(probes.map(([id, url]) =>
        fetchJson(url)
          .then((list) => { platformSessionCounts[id] = Array.isArray(list) ? list.length : 0; })
          .catch(() => { platformSessionCounts[id] = 0; }) // unreachable = treat as empty
      )).then(() => renderAgents());
    }
    ...
    function renderAgents() {
      ...
      platformBar.innerHTML = shownPlatforms
        .map((p) => {
        ...
        .join('')
        + (collapsedPlatforms.length ? `<button class="platform-tab more-tab" id="platformMoreTab" title="暂无会话的平台：${collapsedPlatforms.map((p) => p.label).join('、')} — 点击展开">+${collapsedPlatforms.length}</button>` : '');
    ...
    let promptsSearch = '';
    let promptsHideTrivial = (() => { try { return localStorage.getItem('axr-hide-trivial') !== '0'; } catch { return true; } })();
    ...
    async function loadInsights() {
      ...
      try {
        ...
        insightsPanel.innerHTML = insightsScopeSegHtml('global') + `<div class="insights-empty">Failed to load insights: ${escapeHtml(e.message)}</div>`;
    ...
    function renderInsights(data) {
      if (!data || data.totalSessions === 0) {
        insightsPanel.innerHTML = insightsScopeSegHtml('global') + '<div class="insights-empty">No session data available for the current selection.</div>';
        bindInsightsScopeSeg();
        return;
      }
    ...
    function initToolsAudit() {
      if (toolsAuditChecked || toolsAuditLoading) { renderToolsAudit(); return; }
    ...
    function renderToolsAudit() {
      ...
      const metaHtml = toolsAudit && toolsAudit.generatedAt
      ...
      if (toolsAuditLoading && !toolsAudit) {
        ...
        bodyHtml = '<div style="color:var(--muted);font-size:13px">暂无体检结果 — 点击「运行体检」扫描全部平台的工具使用情况（调用量、错误率、平均耗时、闲置配置）。</div>';
      } else {
        ...
        const moreHtml = tools.length > shown.length
        ...
        bodyHtml = `
          ...
          </table>${moreHtml}` : '<div style="color:var(--muted);font-size:13px">没有工具调用数据</div>'}
          ${unusedHtml}
        `;
      }
      ...
      if (moreBtn) moreBtn.addEventListener('click', () => { toolsAuditShowAll = true; renderToolsAudit(); });
    }
    ...
    function renderAnalysisHtml() {
      ...
      const palette = ['#58a6ff', '#3fb950', '#d29922', '#f778ba', '#a371f7', '#79c0ff', '#56d364', '#e3b341', '#ff7b72', '#8b949e'];
      ...
      if (weeks.length) {
        ...
        const colsHtml = weeks.map(w => {
          const h = Math.max(2, Math.round(w.total / maxTotal * 64));
          const entries = Object.entries(w.topics || {}).filter(([, n]) => n > 0).sort((x, y) => y[1] - x[1]);
          ...
          flopHtml = `
            ...
            <div class="portrait-flop">
              ...
              <div class="portrait-flop-item">
                ...
                ${(c.errorSamples || []).slice(0, 3).map(sm => `<div class="portrait-flop-sample">${escapeHtml(String(sm).replace(/\s+/g, ' ').slice(0, 200))}</div>`).join('')}
              </div>`).join('')}
            </div>`;
        }
      }

      const clustersHtml = displayClusters.map((c, ci) => {
    ...
    function renderPrompts() {
      ...
      document.getElementById('promptsHideTrivial').addEventListener('change', (e) => {
        ...
        try { localStorage.setItem('axr-hide-trivial', promptsHideTrivial ? '1' : '0'); } catch {}
        renderPrompts();
      });
    ...
    function renderFabricList() {
      ...
      fabricList.innerHTML = visible.map(p => `
    ...
    document.getElementById('fabricSelectAll').addEventListener('click', () => {
      fabricVisiblePatterns().forEach(p => { if (!p.imported) fabricSelected.add(p.name); });
      renderFabricList();
    });
    ...
    async function openLibraryHistory(name) {
      ...
      libraryHistoryList.innerHTML = commits.map(c => `
    ...
    function renderHiddenPromptsList() {
      ...
      hiddenPromptsListEl.innerHTML = hiddenPrompts.map(h => `
    ...
    hiddenPromptsRestoreAllBtn.addEventListener('click', () => restoreHiddenPrompts(hiddenPrompts.map(h => h.hash), hiddenPromptsRestoreAllBtn));
    ...
    function renderSessionWindow() {
      ...
      if (!virtMeasured) {
        ...
        if (cards.length) {
          ...
          if (avg > 20) { // ignore hidden-list measurements (offsetHeight 0)
            virtItemHeight = avg;
            virtMeasured = true;
    ...
    function renderSummary() {
      ...
      const tokenBadges = (Object.keys(tokenSummary).length
        ...
        + (sessionCost > 0 ? `<span class="badge" title="按消息 usage.cost.total 合计">💰 ${escapeHtml(formatCost(sessionCost))}</span>` : '');
    ...
    async function exportAsOtlp() {
      ...
      try {
        const data = await fetchJson(`/api/otlp/${encodeURIComponent(state.platform)}/${encodeURIComponent(state.selectedSessionId)}` + dirParam());
        downloadFile(JSON.stringify(data, null, 2), `${state.selectedSessionId}-otlp.json`, 'application/json;charset=utf-8');
      } catch (error) {
    ...
    function scrollToMessage(msgId) {
      ...
      const findEl = () => document.getElementById('message-' + CSS.escape(msgId))
    ...
    async function loadSessions(preserveSelection) {
      ...
      if (state.platform !== 'openclaw' && Array.isArray(state.sessions)) platformSessionCounts[state.platform] = state.sessions.length;
    ...
    async function getSessionChildren() {
      ...
      if (childrenCache.sid === state.selectedSessionId && childrenCache.platform === state.platform) return childrenCache.data;
      try {
      ...
      for (const m of msgs) {
        ...
        for (const c of (m.content || [])) {
          if ((c.type === 'toolCall' || c.type === 'tool_use') && c.id === toolCallId) {
            args = c.arguments ?? c.input ?? null; hasArgs = args != null;
          }
    ...
    cmdkInput.addEventListener('keydown', (e) => {
      if (e.key === 'ArrowDown' || e.key === 'ArrowUp') {
        ...
        cmdkSelected = e.key === 'ArrowDown'
          ? Math.min(cmdkSelected + 1, items.length - 1)
          : Math.max(cmdkSelected - 1, 0);
    ...
    (() => {
      ...
      const check = async () => {
        try {
          ...
          if (bootId === null) { bootId = data.bootId; return; }
          if (data.bootId !== bootId && !notified) {
            notified = true;
            const bar = document.createElement('div');
            bar.style.cssText = 'position:fixed;top:0;left:50%;transform:translateX(-50%);z-index:300;background:#1f6feb;color:#fff;padding:8px 18px;border-radius:0 0 8px 8px;font-size:0.85rem;cursor:pointer;box-shadow:0 2px 12px rgba(0,0,0,0.4)';
            bar.textContent = '🔄 AgentXRay 已更新 — 点击刷新加载新版本';
            bar.addEventListener('click', () => location.reload());
            document.body.appendChild(bar);
          }
        } catch { /* server briefly down during restart — keep polling */ }
      };
      check();
