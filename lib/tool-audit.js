...
const { iterateSessionLines } = require('./insights');
...
// Every file-based platform is auditable (hermes has no session files).
const TOOL_AUDIT_PLATFORMS = Object.values(PLATFORMS)
  .filter((p) => p.collectFiles)
  .map((p) => p.id);
