...
// Every file-based platform (find + parse) is OTLP-exportable; openclaw is
// excluded because its find needs an agent, hermes because it has no files.
const OTLP_PLATFORMS = Object.fromEntries(
  Object.values(PLATFORMS)
    .filter((p) => p.find && p.parse && !p.needsAgent)
    .map((p) => [p.id, { defaultDir: p.defaultDir, find: p.find, parse: p.parse }])
);
