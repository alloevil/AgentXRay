...
const { DSH_DIR, sessionMetaCache } = require('../config');
...
async function parseDshSessionMetadata(filePath, fileName, sessionDirName) {
  try {
  ...
  const data = await _parseDshSessionMetadataRaw(filePath, fileName, sessionDirName);
