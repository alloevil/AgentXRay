...
const { readGeminiMetadataLine, stringifyToolResult: stringifyGeminiToolResult } = require('./platforms/gemini');
...
async function collectSessionFiles(platform, agentName, dirOverride) {
  ...
  if (platform === 'openclaw') {
    ...
    const dir = resolveDir(dirOverride, GEMINI_DIR);
    const projects = await fsp.readdir(dir, { withFileTypes: true }).catch(() => []);
    for (const p of projects) {
      if (!p.isDirectory()) continue;
      ...
      for (const f of entries) {
        if (f.isFile() && /^session-.*\.jsonl$/.test(f.name)) {
          ...
          files.push({ path: filePath, sessionId: (meta && meta.sessionId) || f.name.replace(/\.jsonl$/, '') });
        }
      }
    }
  }
