...
// session-detail routes serve (registry getSession). openclaw additionally
// needs the agent name (needsAgent).
const EXPORT_PLATFORMS = Object.fromEntries(
  Object.values(PLATFORMS).map((p) => [
    p.id,
    {
      defaultDir: p.defaultDir,
      needsAgent: !!p.needsAgent,
      load: (dir, sessionId, opts) => p.getSession(dir, sessionId, opts),
    },
  ])
);
