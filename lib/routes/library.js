...
module.exports = function mountLibraryRoutes(app) {
  ...
  // Suggest a kebab-case slash-command name for a prompt via the configured
  // LLM backend. Always replies { name } — name is null when no backend is
  // available or its output can't be shaped into a valid library name.
  // 400 only for a missing/empty text.
  app.post('/api/library/suggest-name', async (req, res) => {
    ...
    try {
      const raw = await runLlm(prompt, 10_000);
      const name = raw
        .trim()
        .split('\n')
        .pop()
